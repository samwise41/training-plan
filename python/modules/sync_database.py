...
def sync():
    ...
    for key, group in garmin_groups.items():
        if key in log_by_key:
            ...
            # Status will be finalized in final pass, but set here to indicate match found
    ...
    # 1. plannedDuration > 0 AND actualDuration > 0 -> COMPLETED
    # 2. plannedDuration > 0 AND actualDuration = 0 -> MISSED
    # 3. plannedDuration = 0 AND actualDuration > 0 -> UNPLANNED
    print("   🚦 Phase 4: Finalizing Status...")
    for entry in existing_log:
        p_dur = clean_numeric(entry.get('plannedDuration')) or 0
        a_dur = clean_numeric(entry.get('actualDuration')) or 0
