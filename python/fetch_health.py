...
# --- HELPER: INSPECT GARMIN DATA ---
def inspect_garmin_data(garmin_data):
    """Logs unique fields for each activity type to help user identify data points."""
    print_header("🔍 GARMIN DATA FIELD INSPECTION")
    if not garmin_data:
        print("⚠️ No Garmin data to inspect.")
        return
    ...
    for activity in garmin_data:
        ...
        if isinstance(activity.get('activityType'), dict):
            a_type = activity['activityType'].get('typeKey', 'unknown')
        
        if a_type not in seen_types:
            seen_types.add(a_type)
            print(f"\n📂 SAMPLE DATA FOR TYPE: '{a_type}' (Activity ID: {activity.get('activityId')})")
            print("-" * 50)
            
            # Print keys alphabetically
...
def main():
    try:
        ...
        # --- NEW STEP: Inspect Data ---
        inspect_garmin_data(garmin_data)
        # -----------------------------
