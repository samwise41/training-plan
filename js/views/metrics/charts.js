    ...
    return allData
        .map(d => {
            ...
            // 3. RUN Logic (Speed / RPE)
                ...
                if (isRun && spd > 0) {
                    val = spd / rpe;
                    // Convert m/s to Pace for tooltip readability is optional, but raw ratio uses m/s
                    breakdown = `${spd.toFixed(2)} m/s / ${rpe} RPE`;
                    match = true;
                }
            }

            // 4. SWIM Logic (Speed / RPE)
            ...
            if (match && val > 0) {
                return {
                    date: d.date, // Assumes Parser gave a Date object
...
const buildMetricChart = (displayData, fullData, key) => {
    ...
    if (!def) {
        ...
        return `<div class="p-4 text-red-500 text-xs border border-red-500 rounded bg-red-900/20">Error: Definition missing for '${key}'</div>`;
    }
