...
const simpleRender = (text) => {
    ...
    lines.forEach(line => {
        ...
        if (trimmed.startsWith('|')) {
            ...
            // FIX: Split by pipe but keep empty cells. 
            // We slice(1, -1) to remove the leading and trailing empty strings caused by the outer pipes.
            ...
            actualCells.forEach(cell => {
                ...
                const isHeader = cellContent.includes('**') || (inTable && html.endsWith('<table class="w-full text-left border-collapse border border-slate-700 text-sm">')); 
                
                // Simple heuristic: If it's the very first row or has bold text, treat as header
                const tag = isHeader ? 'th' : 'td';
                const style = isHeader ? 'bg-slate-800 font-bold text-white whitespace-nowrap' : 'text-slate-300 min-w-[120px]';
                
                // If cell is empty, render a non-breaking space so borders still show
                const finalContent = cellContent === '' ? '&nbsp;' : parseInline(cellContent);
                
                html += `<${tag} class="border border-slate-600 p-3 ${style}">${finalContent}</${tag}>`;
            });
            html += '</tr>';
            return;
        } 
