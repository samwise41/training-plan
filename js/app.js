...
(async function initApp() {
    ...
    const recordsMod = await safeImport('./views/records/index.js', 'Records');
    ...
    const { renderRecords } = recordsMod || { renderRecords: () => 'Loading Error' };
    ...
    const CONFIG = {
        ...
        RECORDS_FILE: "garmind_data/garmin_records.md",
        RECORDS_JSON: "garmind_data/garmin_records.json", // NEW: Fetch JSON for charts
    ...
    const App = {
        ...
        async init() {
            ...
            const validViews = ['dashboard', 'trends', 'logbook', 'roadmap', 'gear', 'zones', 'readiness', 'metrics', 'health', 'records'];
            ...
            try {
                // Fetch ALL data including JSON records
                const [planRes, gearRes, archiveRes, healthRes, recordsRes, recordsJsonRes] = await Promise.all([
                    ...
                    fetch(`./${CONFIG.RECORDS_FILE}?t=${cacheBuster}`),
                    fetch(`./${CONFIG.RECORDS_JSON}?t=${cacheBuster}`)
                ]);
                ...
                this.healthMd = healthRes.ok ? await healthRes.text() : "";
                this.recordsMd = recordsRes.ok ? await recordsRes.text() : "";
                this.recordsJson = recordsJsonRes.ok ? await recordsJsonRes.json() : null; // Store JSON
        ...
        renderView(view) {
            const titles = { 
                ...
                health: 'Health Metrics', records: 'Personal Records'
            };
            ...
            setTimeout(() => {
                try {
                    ...
                    else if (view === 'zones') content.innerHTML = renderZones(this.planMd, this.recordsJson); // PASS JSON HERE
                    else if (view === 'trends') {
                    ...
                    else if (view === 'readiness') content.innerHTML = renderReadiness(this.allData, this.planMd); 
                    else if (view === 'metrics') content.innerHTML = renderMetrics(this.allData);
                    else if (view === 'health') content.innerHTML = renderHealth(this.healthMd);
                    else if (view === 'records') content.innerHTML = renderRecords(this.recordsMd);
